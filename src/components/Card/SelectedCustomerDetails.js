...
const SelectedCustomerDetails = ({ customer, onOpenPaymentModal, onMarkAsTaken }) => {
  ...
  const totalUnpaidAmount = customer.totalBayar - customer.telahBayar;
  ...
  return (
    <div className="sticky-top py-3" style={{top: '10px', maxHeight: 'calc(100vh - 20px)', display: 'flex', flexDirection: 'column', paddingRight: '10px' }}>
      ...
      <div style={{ flexGrow: 1, overflowY: 'auto', minHeight: 0 }}> {/* minHeight: 0 is important for flex item to shrink properly */}
        {customer.pesanan_payable && customer.pesanan_payable.length > 0 ? (
          customer.pesanan_payable.map((pesanan) => (
