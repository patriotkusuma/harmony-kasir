...
import user from 'assets/img/user/robot23d.png'; // Pastikan path ini benar

const CustomerListItem = ({ customer, isSelected, onClick }) => {
  const amountDue = customer.totalBayar - customer.telahBayar;
  return (
    <Col md="4" lg="3" sm="6" xs="12" className="mt-3"> {/* Adjusted column size for better fit */}
      ...
      >
        <CardImg src={user} style={{ width: '80px', height: '80px', objectFit: 'cover' }} className="mt-3 rounded-circle" alt={customer.nama} />
        {isSelected && (
