...
const Dashboard = (props) => {
  ...
  const getUrutanKerja = async () => {
    try{
      let data = await axios.get("/urutan-kerja",{headers})
      setUrutanKerja(data.data.data.urutan)
    }catch(err){
      Cookies.set();
    }
  }
  const getAmbilKerja = async () => {
    try{
      let data = await axios.get('/ambil-pesanan', {headers})
      setAmbilPesanan(data.data.data.ambil);
    }catch(err){
              ...
              <CardBody>
                ...
                <Table className='align-items-center table-flush' responsive>
                      ...
                      <tbody>
                        ...
                        {urutanKerja != null && urutanKerja.map((urutan, index) => {
                          return(
                            <tr className={index === 0 ? "bg-danger text-light" : ""} key={index}>
                              ...
                              <td>
                                {urutan.detail_pesanan.map((detail, index) => {
                                  return (
                                    <Row key={index}>
                                      <Col>
