...
export const RiwayatPesan = (props) => {
    ...
    const kirimInvoice = async(e) =>{
        ...
        await axios.post(`/kirim-invoice/${e.kode_pesan}`,{
            headers: {
                'Authorization' : `Bearer ${authToken}`
            }
        })
    ...
    const kirimNotif = async(e) =>{
        ...
        await axios.post(`/kirim-notif/${e.kode_pesan}`,{
            headers: {
                'Authorization' : `Bearer ${authToken}`
            }
        })
    ...
    const updateStatus = async(e,currStatus) =>{
        try{
            ...
            await axios.post(`/order/${e.kode_pesan}`, {
                status: currStatus,
                _method: 'patch'
            },{headers}).then((res) =>{
                removeLastParam()
    ...
    },[searchData, authToken, dateFrom, dateTo, status, socket])
            ...
            <Card>
                ...
                <CardBody>
                    ...
                    <CustomTable
                        ...
                        currentUrl = "/order"
                    >   
                    {pesanan != null && pesanan.data.map((pesan,index)=> {
                        return (
                            <tr key={index} className={pesan.status === 'batal' && "bg-info"}>
                                ...
                                <td className={'w-25'}>
                                    ...
                                    {pesan.detail_pesanan.map((detailPesanan, index) => {
                                        return (
                                            <>
                                                <br/>
                                                <strong>
